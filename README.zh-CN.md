...
| 连续追问、核查依据 | 按段落引用原文，代码生成来源标签；保留多轮历史，后台执行不因切换页面而中断 |
...
![结构化回答及可展开的私聊、群聊出处](docs/images/qa-citations.png)
...
[来源归属回归](eval/results/citation-regression/REPORT.md)已通过一次真实 API 复测：
岗位引用群聊，简历建议引用私聊。模型只返回结构化结论与来源编号，由代码校验编号，
再显示真实会话类型、聊天名、发送者和时间。全部来源随回答保存，不再截断为前 12 条。
原始失败案例保留供对照。这是定向回归，不是重新跑完 16 题，也不代表完全消除幻觉。
问答模型需支持严格 JSON Schema 结构化输出。
